...
username = "" # http://www.ttshitu.com/ 账号密码
password = ""
...
class xp_CAPTCHA(IIntruderPayloadGenerator):
    ...
    def getNextPayload(self, payload):  # 这个函数请看下文解释
        ...
        if len(xp_CAPTCHA_list) == 4:
            ...
            data = '{"username":"%s","password":"%s","image":"%s","typeid":"%s"}' % (username, password, CAPTCHA_base64, xp_CAPTCHA_type)
        elif len(xp_CAPTCHA_list) == 1:
            data = '{"username":"%s","password":"%s","image":"%s"}' % (username, password, CAPTCHA_base64)
